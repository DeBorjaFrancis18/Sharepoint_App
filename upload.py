...
def upload_file_in_chunks(ctx, target_folder, file_path, file_name, chunk_size_mb=60):
    ...
    with open(file_path, 'rb') as f:
        ...
        uploaded_file = target_folder.files.create_upload_session(file_name).execute_query()
        upload_id = uploaded_file.unique_id

        # Start upload
        file = target_folder.files.get_by_unique_id(upload_id)
        upload_result = file.start_upload(offset, file_content).execute_query()
        offset += len(file_content)

        # Continue upload
        while offset < file_size:
            file_content = f.read(chunk_size)
            if offset + len(file_content) < file_size:
                upload_result = file.continue_upload(upload_result.upload_id, offset, file_content).execute_query()
            else:
                file = file.finish_upload(upload_result.upload_id, offset, file_content).execute_query()
            offset += len(file_content)
